      ...
      <div className="bg-slate-900 rounded-xl border border-slate-700/50 shadow-xl w-full max-w-lg max-h-[70vh] flex flex-col">
        <div className="px-5 py-4 border-b border-slate-700/30">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={handleClose} className="p-1 text-slate-400 hover:bg-slate-800 rounded-lg transition"><X className="w-4 h-4" /></button>
          </div>

          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
            <input value={searchQuery} onChange={handleSearchInput} placeholder="Search users to add..." className="w-full pl-9 pr-3 py-2 bg-slate-800 border border-slate-700/50 rounded-lg text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder:text-slate-500" />
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {isSearching ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-sm text-slate-400">{searchQuery ? "No users found" : "Search for users"}</p>
            </div>
          ) : (
            <div className="space-y-1">
              {searchResults.map((user) => {
                ...
                return (
                  <div key={user._id} className={`flex items-center justify-between p-3 rounded-lg transition ${alreadyMember ? "bg-slate-800/30 opacity-50" : selected ? "bg-indigo-500/10" : "hover:bg-slate-800/50"}`}>
