...
export default function PresentationEditor({ content, onContentChange, isReadOnly, slideTheme, onThemeChange, remotePatch, onSlideUpdate }) {
  ...
  /* Keep in sync when parent content changes (load from API / remote full-state); skip our own emits */
  useEffect(() => {
    if (content && content === lastSerializedRef.current) return;
    try {
      if (content) {
  ...
  const sync = useCallback((s, patch = null) => {
    setSlides(s);
    const serialized = JSON.stringify({ slides: s });
    lastSerializedRef.current = serialized;
    if(onContentChange) onContentChange(serialized);
    pushH(s);
    if(onSlideUpdate) {
  ...
  const undo = () => { 
    ...
    const ser = JSON.stringify({ slides:ns });
    lastSerializedRef.current = ser;
    if(onContentChange) onContentChange(ser); 
